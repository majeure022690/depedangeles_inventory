...
1. **`StakeholderProfile` and `InternetConnectivitySurvey` are singletons** (exactly one row each, ever — see `docs/architecture.md`'s singleton pattern). A many-to-many pivot table's entire value proposition — efficient joins and integrity across *many* parent rows — is moot when the "many" side is permanently fixed at one row. A pivot here would be pure ceremony: a join table that will only ever contain rows for a single `stakeholder_profile_id` / `internet_connectivity_survey_id`.
