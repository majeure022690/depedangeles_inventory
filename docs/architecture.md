...
### 3. One row per tenant (Stakeholder Profile, Internet Connectivity Survey)

Used for records that describe one office/tenant — every school and division-level office/unit (`Office`) gets its own row, filled in by that office's own users — rather than a global fact (there's no single "the" record) or a list of independent items. Both resources using this pattern today started life as a true global singleton (exactly one row for the whole application, always reached via `Model::firstOrCreate([])` with no `{id}` route parameter) before the app grew to track multiple schools under the division rather than a single division office: Stakeholder Profile converted first (2026-07-14), Internet Connectivity Survey followed one day later (2026-07-15). Neither controller has `create()`/`store()`/`destroy()` — a row is always lazily `Model::firstOrCreate(['office_id' => ...])`'d, never explicitly created or deleted — but each gains an `index()` (an admin-only cross-office list) alongside `edit()`/`update()`, both always scoped to one specific `{office}` route parameter.

Access is scoped, not just the row: `StakeholderProfilePolicy`/`InternetConnectivitySurveyPolicy` each grant `view`/`update` either to the acting user's own office (`$user->office_id === $model->office_id`, holding the resource's own `.view`/`.edit` permission) or unconditionally to a `.view_all` holder (division-level oversight, seeded only to `admin`) — the same permission name (e.g. `stakeholder_profile.view`) means something different depending on *whose* `office_id` it's checked against, which is why the Policy — not just a permission-string check — is what actually enforces the boundary here. Both Policies authorize a transient (unsaved) model instance with `office_id` already set on `edit()`/`update()`, checked *before* touching the database, so a wrong-office (or `pending`-role) user's request 403s without creating that office's row as a side effect.

What replaced the old race guard: the DB-level `singleton_guard` column both tables once carried (an `unsignedTinyInteger` defaulting to `1` with a `unique()` index, added retroactively by a 2026-07-11 migration to close a race a security review flagged — two concurrent first-load requests each passing the `SELECT` half of `firstOrCreate()` and both `INSERT`ing a duplicate row) no longer applies to either table now that there's no longer "exactly one row globally" to protect. It's replaced on both by a plain `unique` index on `office_id` itself. MySQL/MariaDB unique indexes allow unlimited `NULL`s, but that's moot here since `office_id` is `NOT NULL` on both tables; every row either resource creates always belongs to a specific office, so the unique index alone is the complete guarantee ("at most one row per office").

**Internet Connectivity Survey's discarded singleton row:** unlike Stakeholder Profile's old singleton row (empty junk, nothing lost by dropping it), Internet Connectivity Survey's one pre-existing row held real seeded survey answers (`InternetConnectivitySurveySeeder`, from the source Excel workbook's "Internet Connectivity" sheet). Its 2026-07-15 conversion migration still deletes that row outright rather than attaching it to a specific office — an explicit product-owner decision, since there was no reliable way to know which office the workbook's single division-wide answer set was ever meant to represent. `InternetConnectivitySurveySeeder` and its data file were removed in the same change; every office's survey now starts empty, lazily created on first edit, same as Stakeholder Profile.

**Internet Connectivity Survey's live-computed aggregates:** the survey's summary panel (Total ISPs, Total Cost/month, Total Amount Spent, Total Projected Expenditure, Rooms Covered Admin/Classroom, Total Access Points) mirrors fields the source Excel sheet marked "Protected, source data from..." — i.e., derived values, not user input. None of these are columns on `internet_connectivity_surveys`; there's no FK to `isp_accounts` / `isp_subscription_costs` either — and `isp_accounts` itself has no `office_id` column, so these totals stay division-wide across every office's edit page rather than scoped to the office being viewed (a known limitation of the source data, not something the office-scoping conversion introduced or could fix). `InternetConnectivitySurveyController::computeAggregates()` queries `IspAccount`/`IspSubscriptionCost` fresh on every `edit()` load and passes the result as a separate, read-only `aggregates` Inertia prop — never part of the writable Form Request payload, never cached or stored. The one exception on that table is `rooms_other_use`, which genuinely is user-entered in the source and so is a real column. Since the 2026-07-15 conversion, `aggregates` is additionally `null` unless the requesting user separately holds `isp_accounts.view` — a post-conversion security-review finding, since `InternetConnectivitySurveyPolicy` authorizes the survey itself, not ISP account data, and a custom role could otherwise hold `internet_connectivity.view` without `isp_accounts.view` and still see division-wide ISP budget totals. See [`docs/features/internet-connectivity-survey.md`](features/internet-connectivity-survey.md) for the full detail.
...
**`Office` (2026-07-15) is a fourth shape entirely: a dedicated CRUD resource, not part of this reference-data system at all.** Its column shape (`office_name`, `office_type`, `school_id`, `address`, `region`, `district`, `division`, `contact_number`, `email`, `is_active`) fits neither Tier 1 nor Tier 2, and it's the FK backbone `users`/`stakeholder_profiles`/`internet_connectivity_surveys` all point at, not a closed lookup vocabulary — so it gets its own `OfficeController`/`OfficePolicy`/Form Requests/`resources/js/pages/offices/`, admin-only permissions (`office.view`/`.create`/`.edit`/`.delete`), and DB-level `restrictOnDelete()` FKs instead of the reference-data system's guarded-delete pattern. Reachable from the "Libraries" sidebar entry as a distinct "Institutional data" section, not folded into the Tier 1/Tier 2 grid. See [`docs/features/offices.md`](features/offices.md).
