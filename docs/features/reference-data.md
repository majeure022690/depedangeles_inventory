...
- **No pivot tables for the nine JSON-array multi-select columns**, for two independent reasons: `StakeholderProfile`/`InternetConnectivitySurvey` are singletons (a pivot's whole value proposition is moot when the "many" side is permanently one row), and `Personnel.fund_source`'s Tier 2 vocabulary has no evidence of a real "find all personnel funded by X" query need today (YAGNI — promote to a pivot if that need surfaces on evidence, not speculatively).
