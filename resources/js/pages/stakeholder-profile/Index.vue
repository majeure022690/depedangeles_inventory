...
import type { StakeholderProfileFilters, StakeholderProfileOfficeListItem } from '@/types/stakeholder-profile';

const props = defineProps<{
    offices: Paginated<StakeholderProfileOfficeListItem>;
    filters: StakeholderProfileFilters;
}>();

defineOptions({
    layout: {
        breadcrumbs: [{ title: 'Stakeholder Profile', href: stakeholderProfiles.index() }],
    },
});

const { filters, update } = useTableFilters(stakeholderProfiles.index.url(), {
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4">
        <Heading
            ...
            description="Every school and division-level office's stakeholder profile — one per office, filled in by that office's own users."
        />
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y">
                    ...
                    <tr v-for="row in props.offices.data" :key="row.id">
                        ...
                        <td class="px-4 py-3">
                            <Badge :variant="row.has_profile ? 'secondary' : 'outline'">
                                {{ row.has_profile ? 'Submitted' : 'Not started' }}
                            </Badge>
                        </td>
